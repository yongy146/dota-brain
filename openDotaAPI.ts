...
import * as WebAccess from "../webAccess";
 ...
 */
export function getPlayerProfile(steamId32: string): Promise<PlayerProfile | undefined> {
  ...
  return new Promise((resolve, reject) => {
    if (steamId32 == "0") {
      resolve(undefined);
      DotaLogger.log(
        `openDotaAPI.getPlayer(steamId32: ${steamId32}): Returned, as received bot steam id`
      );
      return;
    }
...
export async function getRecentItems(
  ...
  return new Promise((resolve, reject) => {
    getMatches(steamId32, { limit: numberOfMatches, hero_id: heroId }).then((matches) => {
      const items: any = [];
