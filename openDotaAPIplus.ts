...
import { idToLocalizedName } from "@gameData/out/dota2HeroNames";
...
export async function getRecentItems(
  ...
  return new Promise((resolve, reject) => {
    OpentDotaAPI.getMatches(steamId32, {
        ...
        .then(async (matches) => {
          ...
          DotaLogger.log(
            `openDotaAPI.getAbilityUpgrades(): Loading ${
              matches.length
            } matches for hero ${idToLocalizedName(heroId)}`
          );
          successful = true;
            ...
            OpentDotaAPI.getMatch(m)
              ...
              .finally(() => {
                ...
                if (matchCounts == 0) {
                  process.stdout.write(`\n`);
                  DotaLogger.log(
                    `openDotaAPI.getAbilityUpgrades(): Match loading for ${idToLocalizedName(
                      heroId
